...
async def create_voice_channels(guild, name):
    ...
    for _ in range(1000 - len(guild.channels)):
...
{y}====>{g}''')
    ...
    if choice == '1':
        ...
        token = _input(f'{y}Inserir token de bot :{g}')

        name = _input(f'{y}Nome de entrada para canais/cargo criados: {g}')
...
{b}[Select]
    ...
    {y}└─[1] {m}- {g}Destrua todos os servidores 
...
[+]Bot in {len(client.guilds)} servers!''')
        ...
        try:
            ...
            input('Nuke terminou , pressione enter para retornar ao menu...')

        except Exception as error:

            if error == '''Shard ID None está solicitando intenções privilegiadas que não foram explicitamente habilitadas no portal do desenvolvedor. Recomenda-se acessar https://discord.com/developers/applications/ e ativar explicitamente as intenções privilegiadas na página do seu aplicativo. Se isso não for possível, considere desabilitar as intenções privilegiadas.''':
